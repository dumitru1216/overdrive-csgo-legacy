...
void netvar_sys::init( )
{
	...
	for ( auto clientclass = g_csgo.client( )->get_all_classes( );
		clientclass != nullptr;
...
netvar_sys::netvar_table netvar_sys::LoadTable( RecvTable* recvTable )
{
...
void netvar_sys::Dump( std::ostream& stream )
{
...
void netvar_sys::DumpTable( std::ostream& stream, const netvar_table& table, uint32_t indentation )
{
...
uint32_t netvar_sys::GetOffset( const std::string& tableName, const std::string& propName )
{
	auto result = 0u;
	for ( const auto& table : database ) {
...
uint32_t netvar_sys::GetOffset( const netvar_sys::netvar_table& table, const std::string& propName )
{
	for ( const auto& prop : table.child_props ) {
...
RecvProp* netvar_sys::GetNetvarProp( const std::string& tableName, const std::string& propName )
{
...
RecvProp* netvar_sys::GetNetvarProp( const netvar_sys::netvar_table& table, const std::string& propName )
{
	for ( const auto& prop : table.child_props ) {
		if ( strncmp( prop->m_pVarName, propName.data( ), propName.size( ) ) == 0 ) {
			return prop;
